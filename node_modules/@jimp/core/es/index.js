...
function (_EventEmitter) {
  ...
  function Jimp() {
    ...
    _defineProperty(_assertThisInitialized(_this), "writeAsync", function (path) {
      return promisify(_this.write, _assertThisInitialized(_this), path);
    });

    _defineProperty(_assertThisInitialized(_this), "getBase64Async", function (mime) {
      return promisify(_this.getBase64, _assertThisInitialized(_this), mime);
    });

    _defineProperty(_assertThisInitialized(_this), "getBuffer", getBuffer);

    _defineProperty(_assertThisInitialized(_this), "getBufferAsync", getBufferAsync);
    ...
    if (typeof args[0] === 'number' && typeof args[1] === 'number' || parseInt(args[0], 10) && parseInt(args[1], 10)) {
      ...
      if (typeof cb !== 'function') {
        return _possibleConstructorReturn(_this, throwError.call(_assertThisInitialized(_this), 'cb must be a function', finish));
      }
      ...
      loadFromURL(args[0], function (err, data) {
        if (err) {
          return throwError.call(_assertThisInitialized(_this), err, finish);
        }
      ...
      if (extraConstructor) {
        ...
        return _possibleConstructorReturn(_this, throwError.call(_assertThisInitialized(_this), 'No matching constructor overloading was found. ' + 'Please see the docs for how to call the Jimp constructor.', finish));
      }
    }

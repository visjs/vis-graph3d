...
function (_EventEmitter) {
  ...
  function Jimp() {
    ...
    _defineProperty(_assertThisInitialized(_this), "bitmap", emptyBitmap);

    _defineProperty(_assertThisInitialized(_this), "_background", 0x00000000);

    _defineProperty(_assertThisInitialized(_this), "_originalMime", Jimp.MIME_PNG);

    _defineProperty(_assertThisInitialized(_this), "_exif", null);

    _defineProperty(_assertThisInitialized(_this), "_rgba", true);

    _defineProperty(_assertThisInitialized(_this), "writeAsync", function (path) {
      return (0, _promisify["default"])(_this.write, _assertThisInitialized(_this), path);
    });

    _defineProperty(_assertThisInitialized(_this), "getBase64Async", function (mime) {
      return (0, _promisify["default"])(_this.getBase64, _assertThisInitialized(_this), mime);
    });

    _defineProperty(_assertThisInitialized(_this), "getBuffer", _imageBitmap.getBuffer);

    _defineProperty(_assertThisInitialized(_this), "getBufferAsync", _imageBitmap.getBufferAsync);

    _defineProperty(_assertThisInitialized(_this), "getPixelColour", _this.getPixelColor);

    _defineProperty(_assertThisInitialized(_this), "setPixelColour", _this.setPixelColor);

    var jimpInstance = _assertThisInitialized(_this);
    ...
    if (typeof args[0] === 'number' && typeof args[1] === 'number' || parseInt(args[0], 10) && parseInt(args[1], 10)) {
      ...
      if (typeof cb !== 'function') {
        return _possibleConstructorReturn(_this, _utils.throwError.call(_assertThisInitialized(_this), 'cb must be a function', finish));
      }
      ...
      finish(null, _assertThisInitialized(_this));
      ...
      loadFromURL(args[0], function (err, data) {
        if (err) {
          return _utils.throwError.call(_assertThisInitialized(_this), err, finish);
        }
      ...
      if (extraConstructor) {
        new Promise(function (resolve, reject) {
          ...
          return (_extraConstructor$run = extraConstructor.run).call.apply(_extraConstructor$run, [_assertThisInitialized(_this), resolve, reject].concat(args));
        }).then(function () {
          return finish(null, _assertThisInitialized(_this));
        })["catch"](finish);
      } else {
        return _possibleConstructorReturn(_this, _utils.throwError.call(_assertThisInitialized(_this), 'No matching constructor overloading was found. ' + 'Please see the docs for how to call the Jimp constructor.', finish));
      }
    }

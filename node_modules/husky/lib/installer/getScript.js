...
// Render script
const render = ({ createdAt, huskyHomepage, huskyVersion, pkgDirectory, pkgHomepage, pmName, relativeUserPkgDir }) => `#!/bin/sh
 ...
 */
function default_1({ relativeUserPkgDir, pmName }) {
    const pkgHomepage = process.env.npm_package_homepage;
    const pkgDirectory = process.env.PWD;
    const { homepage: huskyHomepage, version: huskyVersion } = read_pkg_1.readPkg(path_1.default.join(__dirname, '../..'));
    const createdAt = new Date().toLocaleString();
    if (!['npm', 'pnpm', 'yarn'].includes(pmName)) {
        throw new Error(`Unknown package manager: ${pmName} (npm_config_user_agent: ${process.env.npm_config_user_agent})`);
    }
    const normalizedPath = slash_1.default(relativeUserPkgDir);
    // Render script
    return render({
        createdAt,
        huskyHomepage,
